...
- ~Expr~ (~70): full object/player/combat/set/arithmetic/binding read surface.
...
1. *Split stack* (see Known seams): spells in ~objects~, abilities in ~abilities~;
   "counter any spell or ability" still needs a Union of TargetSpecs. *Only structural gap left.*
...
Next: only the split-stack seam remains structurally open. Per-card ~eff_*~ →
~IrSpellMode~ translation continues opportunistically. Census review due every 20 cards /
on any new structural variant.
